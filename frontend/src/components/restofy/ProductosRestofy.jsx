import React, { useEffect, useState } from 'react';
import { useAuth } from '../../contexts/AuthContext';
import RestofyService from '../../services/restofyService';
import { obtenerCredencialesRestofy } from '../../utils/restofyUtils';
import './RestofyComponents.css';

const ProductosRestofy = ({ empresaId, empresa }) => {
  const { token } = useAuth();
  const [productos, setProductos] = useState([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);
  const [page, setPage] = useState(1);
  const [tieneRestofy, setTieneRestofy] = useState(false);
  const [restofyUrl, setRestofyUrl] = useState(null);
  const [restofyToken, setRestofyToken] = useState(null);
  const [credencialesCargadas, setCredencialesCargadas] = useState(false);

  // Obtener credenciales de Restofy desde la empresa
  useEffect(() => {
    const cargarCredenciales = async () => {
      if (!empresaId) {
        console.warn('⚠️ ProductosRestofy: No hay ID de empresa');
        return;
      }

      console.log('🔑 ProductosRestofy: Cargando credenciales de Restofy', {
        empresaId,
        tieneEmpresa: !!empresa
      });

      const credenciales = await obtenerCredencialesRestofy(empresaId, empresa, token);
      
      if (credenciales.tieneRestofy) {
        console.log('✅ ProductosRestofy: Credenciales cargadas exitosamente');
        setRestofyUrl(credenciales.restofyUrl);
        setRestofyToken(credenciales.restofyToken);
        setTieneRestofy(true);
        setError(null);
      } else {
        console.warn('⚠️ ProductosRestofy: No se pudieron obtener credenciales', {
          error: credenciales.error
        });
        setTieneRestofy(false);
        setError(credenciales.error);
      }
      
      setCredencialesCargadas(true);
    };

    cargarCredenciales();
  }, [empresaId, empresa, token]);

  const cargarProductos = async () => {
    if (!restofyUrl || !restofyToken) {
      console.warn('⚠️ ProductosRestofy: Faltan credenciales para cargar productos', {
        tieneUrl: !!restofyUrl,
        tieneToken: !!restofyToken
      });
      return;
    }

    console.log('📦 ProductosRestofy: Cargando productos', { page });

    setLoading(true);
    setError(null);

    try {
      // Llamar directamente a la API de Restofy
      const result = await RestofyService.obtenerProductos(restofyUrl, restofyToken, page);

      if (result.success) {
        const responseData = result.data;
        if (responseData && responseData.status === 1) {
          const productosObtenidos = responseData.content || [];
          console.log('✅ ProductosRestofy: Productos obtenidos exitosamente', {
            cantidad: productosObtenidos.length,
            page
          });
          setProductos(productosObtenidos);
          setTieneRestofy(true);
          setError(null);
        } else {
          console.warn('⚠️ ProductosRestofy: Respuesta con error de la API', {
            status: responseData.status,
            error: responseData.error
          });
          setProductos([]);
          setError(responseData.error || 'No se pudieron obtener los productos');
        }
      } else {
        console.error('❌ ProductosRestofy: Error al obtener productos', {
          error: result.error,
          status_code: result.status_code
        });
        setProductos([]);
        setError(result.error);
        setTieneRestofy(result.tiene_restofy !== undefined ? result.tiene_restofy : true);
      }
    } catch (err) {
      console.error('❌ ProductosRestofy: Excepción al cargar productos', {
        error: err.message,
        stack: err.stack
      });
      setProductos([]);
      setError('Error al cargar los productos: ' + err.message);
    } finally {
      setLoading(false);
    }
  };

  useEffect(() => {
    if (empresaId && credencialesCargadas) {
      console.log('📦 ProductosRestofy: Inicializando carga de productos');
      setPage(1);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [empresaId, credencialesCargadas]);

  useEffect(() => {
    if (empresaId && restofyUrl && restofyToken && credencialesCargadas && tieneRestofy && page > 0) {
      console.log('🔄 ProductosRestofy: Cargando productos', { page });
      cargarProductos();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [page, restofyUrl, restofyToken, credencialesCargadas, tieneRestofy]);

  const formatearMoneda = (valor) => {
    if (!valor) return '$0';
    const num = parseFloat(valor);
    return new Intl.NumberFormat('es-CO', {
      style: 'currency',
      currency: 'COP',
      minimumFractionDigits: 0
    }).format(num);
  };

  // Mostrar estado de carga de credenciales
  if (!credencialesCargadas) {
    return (
      <div className="restofy-container">
        <div className="restofy-loading">
          <span className="loading-icon">🔄</span>
          <p>Cargando credenciales de Restofy...</p>
        </div>
      </div>
    );
  }

  if (!tieneRestofy) {
    return (
      <div className="restofy-container">
        <div className="restofy-error">
          <div className="error-icon">⚠️</div>
          <h3>Restofy no configurado</h3>
          <p>Esta empresa no tiene integración con Restofy configurada.</p>
          <p>Configure la URL y token de Restofy en la configuración de la empresa.</p>
          {error && (
            <p className="error-detail" style={{ marginTop: '10px', color: '#c33', fontSize: '0.9em' }}>
              {error}
            </p>
          )}
        </div>
      </div>
    );
  }

  return (
    <div className="restofy-container">
      <div className="restofy-header">
        <h3>📦 Productos - {empresa?.razon_social || 'Empresa'}</h3>
        <p className="restofy-subtitle">Listado de productos de Restofy</p>
      </div>

      {error && (
        <div className="restofy-error-message">
          <span className="error-icon">❌</span>
          <p>{error}</p>
        </div>
      )}

      {loading && (
        <div className="restofy-loading">
          <span className="loading-icon">🔄</span>
          <p>Cargando productos...</p>
        </div>
      )}

      {!loading && !error && productos.length === 0 && (
        <div className="restofy-empty">
          <span className="empty-icon">📭</span>
          <h4>No se encontraron productos</h4>
          <p>No hay productos disponibles en Restofy.</p>
        </div>
      )}

      {!loading && !error && productos.length > 0 && (
        <>
          <div className="restofy-table-container">
            <table className="restofy-table">
              <thead>
                <tr>
                  <th>ID</th>
                  <th>Código</th>
                  <th>Nombre</th>
                  <th>Descripción</th>
                  <th>Precio</th>
                  <th>Categoría</th>
                  <th>Estado</th>
                </tr>
              </thead>
              <tbody>
                {productos.map((producto) => (
                  <tr key={producto.id}>
                    <td>{producto.id || 'N/A'}</td>
                    <td>
                      <strong>{producto.code || producto.codigo || 'N/A'}</strong>
                    </td>
                    <td>{producto.name || producto.nombre || 'N/A'}</td>
                    <td className="table-description">
                      {producto.description || producto.descripcion || '-'}
                    </td>
                    <td className="table-price">{formatearMoneda(producto.price || producto.precio)}</td>
                    <td>{producto.category_name || producto.categoria || '-'}</td>
                    <td>
                      {producto.active !== undefined ? (
                        producto.active ? (
                          <span className="badge badge-success">Activo</span>
                        ) : (
                          <span className="badge badge-danger">Inactivo</span>
                        )
                      ) : (
                        <span className="badge badge-info">N/A</span>
                      )}
                    </td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>

          <div className="restofy-pagination">
            <button
              onClick={() => setPage(page - 1)}
              disabled={page <= 1 || loading}
              className="btn btn-secondary"
            >
              ← Anterior
            </button>
            <span className="pagination-info">Página {page}</span>
            <button
              onClick={() => setPage(page + 1)}
              disabled={productos.length === 0 || loading}
              className="btn btn-secondary"
            >
              Siguiente →
            </button>
          </div>
        </>
      )}
    </div>
  );
};

export default ProductosRestofy;
